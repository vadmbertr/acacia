...
Dtoast <- readRDS(paste0("3b_featureselection_step1/TOAST/",date,"_D_toast_restricted.rds"))
toast_hvmet <- readRDS("../2210_1SB_featselec/feat_selec/hvg.rds")
toast_hvmet = lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) toast_hvmet[[10*(lot-1)+sim]])
})
toast_feat <- list(toast_met=readRDS("../2210_1SB_featselec/feat_selec/TOAST/met_restricted.rds"),
                   toast_rna=readRDS("../2210_1SB_featselec/feat_selec/TOAST/rna_restricted.rds"))
...
Treftoast <- lapply(seq(n_lot), function(lot)
  lapply(seq(n_sims), function(sim) {
    ...
    mat <- rbind(mat1,mat2)
    mat}))
rm(Tref1_toast,Tref2_toast)

Dcimpleg <- readRDS(paste0("3b_featureselection_step1/CimpleG/",date,"_D_cimpleg_restricted.rds"))
cimpleg_feat <- readRDS("../2210_1SB_featselec/feat_selec/CimpleG/met_restricted.rds")
...
tmp=lapply(seq(n_lot), function (lot) {
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    lapply(deconv_meth, function(meth) {
      if (!file.exists(paste0("3b_featureselection_step2/TOAST/deconv_restricted/",
                              date,
                              "_",name_data[lot],
                              "_Apred_",
                              meth,
        ...
        saveRDS(deconv_res,paste0("3b_featureselection_step2/TOAST/deconv_restricted/",
                              date,
                              "_",name_data[lot],
                              "_Apred_",
                              meth,
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    res = lapply(deconv_meth, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/TOAST/deconv_restricted/",
                             ...
                             "_",sim_txt,
                             sim,".rds"))) {
        A_pred=readRDS(paste0("3b_featureselection_step2/TOAST/deconv_restricted/",
                              date,"_",name_data[lot],
                              "_Apred_",
                              meth,
                              "_",sim_txt,
...
tmp=lapply(seq(n_lot), function (lot) {
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    print(paste0("Running simulation n°",sim,"/",n_sims))
    lapply(deconv_meth, function(meth) {
      if (!file.exists(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/",
                              date,
                              "_",name_data[lot],
                              "_Apred_",
                              meth,
        ...
        if (!all(is.na(Dcimpleg[[lot]]))) {
          ...
          ref_profiles=ref_profiles[,sort(colnames(ref_profiles))]
          D=Dcimpleg[[lot]][[sim]]
          rownames(D)=rownames(ref_profiles)
          deconv_res=run_toast_deconvolution(meth, D, ref_profiles, Atrue[[lot]][[sim]])
        }
        else {deconv_res=list(res=NA,time_elapsed=NA)}
        saveRDS(deconv_res,paste0("3b_featureselection_step2/CimpleG/deconv_restricted/",
                              date,
                              "_",name_data[lot],
                              "_Apred_",
                              meth,
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    res = lapply(deconv_meth, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/",
                             ...
                             "_",sim_txt,
                             sim,".rds"))) {
        A_pred=readRDS(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/",
                              date,"_",name_data[lot],
                              "_Apred_",
                              meth,
                              "_",sim_txt,
