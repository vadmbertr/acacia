...
D_hv = readRDS(paste0("3a_rawconcatenation_step1/",
                      date,
                      "_DT_hv.rds"))$D
T_hv = readRDS(paste0("3a_rawconcatenation_step1/",
                      date,
                      "_DT_hv.rds"))$Tref
...
tmp=lapply(seq(n_lot), function (lot) {
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    lapply(deconv_meth, function(meth) {
      if (!file.exists(paste0("3a_rawconcatenation_step2_nonorm/deconv/",
                              date,
                              "_",name_data[lot],
                              "_Apred_",
                              meth,
        ...
        ref_profiles=as.matrix(T_hv[[lot]][[sim]])
        ref_profiles=ref_profiles[,sort(colnames(ref_profiles))]
        deconv_res=run_concat_deconvolution(meth, D_hv[[lot]][[sim]], ref_profiles, Atrue[[lot]][[sim]])
        saveRDS(deconv_res,paste0("3a_rawconcatenation_step2_nonorm/deconv/",
                                  date,
                                  "_",name_data[lot],
                                  "_Apred_",
                                  meth,
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    res = lapply(deconv_meth, function(meth) {
      if (file.exists(paste0("3a_rawconcatenation_step2_nonorm/deconv/",
                             ...
                             "_",sim_txt,
                             sim,".rds"))) {
        A_pred=readRDS(paste0("3a_rawconcatenation_step2_nonorm/deconv/",
                              date,"_",name_data[lot],
                              "_Apred_",
                              meth,
                              "_",sim_txt,
...
saveRDS(score_perf_df, paste0("3a_rawconcatenation_step2_nonorm/perf_scores/",
                              date,
                              "_scores.rds"))