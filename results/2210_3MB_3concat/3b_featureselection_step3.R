...
score_fs_toast <- readRDS("3b_featureselection_step2/TOAST/perf_scores/221101_scores.rds")
score_fs_toast$datatype = "TOAST"
score_fs_cimpleg <- readRDS("3b_featureselection_step2/CimpleG/perf_scores/221101_scores.rds")
score_fs_cimpleg$datatype = "CimpleG"
score_fs <- bind_rows(score_fs_toast,score_fs_cimpleg)
rm(score_fs_toast,score_fs_cimpleg)

score_restricted_toast <- readRDS("3b_featureselection_step2/TOAST/perf_scores/221101_scores_restricted.rds")
score_restricted_toast$datatype = "TOAST_r"
score_restricted_cimpleg <- readRDS("3b_featureselection_step2/CimpleG/perf_scores/221101_scores_restricted.rds")
...
time_fs_t=lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) {
    ...
    sapply(deconv_methods, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/TOAST/deconv/221101_",name_data[lot],
                               "_Apred_",
                               meth,
                               "_",sim_txt,sim,".rds"))) {
          tmp=readRDS(paste0("3b_featureselection_step2/TOAST/deconv/221101_",name_data[lot],
                             "_Apred_",
                             meth,
                             "_",sim_txt,sim,".rds"))$time_elapsed
          tmp0=readRDS(paste0("3b_featureselection_step2/TOAST/deconv/221101_",name_data[lot],
                              "_Apred_",
                              meth,
          ...
          if (is.list(tmp)) {
            ...
            saveRDS(list(res=tmp0,time_elapsed=tmp),paste0("3b_featureselection_step2/TOAST/deconv/221101_",name_data[lot],
                                                           "_Apred_",
                                                           meth,
                                                           "_",sim_txt,sim,".rds"))
          }
...
time_fs_c=lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) {
    sim_txt <- ifelse(length(strsplit(as.character(sim),"")[[1]])==1,'sim0','sim')
    sapply(deconv_methods, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/CimpleG/deconv/221101_",name_data[lot],
                             "_Apred_",
                             meth,
                             "_",sim_txt,sim,".rds"))) {
        tmp=readRDS(paste0("3b_featureselection_step2/CimpleG/deconv/221101_",name_data[lot],
                           "_Apred_",
                           meth,
                           "_",sim_txt,sim,".rds"))$time_elapsed
        tmp0=readRDS(paste0("3b_featureselection_step2/CimpleG/deconv/221101_",name_data[lot],
                            "_Apred_",
                            meth,
                            "_",sim_txt,sim,".rds"))$res
        if (is.list(tmp)) {
          tmp=tmp[[4]]
          tmp=as.numeric(strsplit(tmp," ")[[1]][1])
          saveRDS(list(res=tmp0,time_elapsed=tmp),paste0("3b_featureselection_step2/CimpleG/deconv/221101_",name_data[lot],
                                                         "_Apred_",
                                                         meth,
                                                         "_",sim_txt,sim,".rds"))
        }
...
time_restricted_t=lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) {
    sim_txt <- ifelse(length(strsplit(as.character(sim),"")[[1]])==1,'sim0','sim')
    sapply(deconv_methods, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/TOAST/deconv_restricted/221101_",name_data[lot],
                             "_Apred_",
                             meth,
                             "_",sim_txt,sim,".rds"))) {
        tmp=readRDS(paste0("3b_featureselection_step2/TOAST/deconv_restricted/221101_",name_data[lot],
                           "_Apred_",
                           meth,
                           "_",sim_txt,sim,".rds"))$time_elapsed
        tmp0=readRDS(paste0("3b_featureselection_step2/TOAST/deconv_restricted/221101_",name_data[lot],
                            "_Apred_",
                            meth,
                            "_",sim_txt,sim,".rds"))$res
        if (is.list(tmp)) {
          tmp=tmp[[4]]
          tmp=as.numeric(strsplit(tmp," ")[[1]][1])
          saveRDS(list(res=tmp0,time_elapsed=tmp),paste0("3b_featureselection_step2/TOAST/deconv_restricted/221101_",name_data[lot],
                                                         "_Apred_",
                                                         meth,
                                                         "_",sim_txt,sim,".rds"))
        }
...
time_restricted_c=lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) {
    sim_txt <- ifelse(length(strsplit(as.character(sim),"")[[1]])==1,'sim0','sim')
    sapply(deconv_methods, function(meth) {
      if (file.exists(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/221101_",name_data[lot],
                             "_Apred_",
                             meth,
                             "_",sim_txt,sim,".rds"))) {
        tmp=readRDS(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/221101_",name_data[lot],
                           "_Apred_",
                           meth,
                           "_",sim_txt,sim,".rds"))$time_elapsed
        tmp0=readRDS(paste0("3b_featureselection_step2/CimpleG/deconv_restricted/221101_",name_data[lot],
                            "_Apred_",
                            meth,
                            "_",sim_txt,sim,".rds"))$res
        if (is.list(tmp)) {
          tmp=tmp[[4]]
          tmp=as.numeric(strsplit(tmp," ")[[1]][1])
          saveRDS(list(res=tmp0,time_elapsed=tmp),paste0("3b_featureselection_step2/CimpleG/deconv_restricted/221101_",name_data[lot],
                                                         "_Apred_",
                                                         meth,
                                                         "_",sim_txt,sim,".rds"))
        }
...
rank$feat_selec <- c(rep("CimpleG",10),rep("TOAST",10))
rank$datatype <- sapply(rank$candidate, function(x) {
  inf = strsplit(x," ")[[1]][1]
  switch(inf,
         "CimpleG"="FS",
         "TOAST"="FS",
         "CimpleG_r"="restricted",
         "TOAST_r"="restricted",
         "met"="MET",
         "rna"="RNA", stop("error"))
})
rank$datatype <- factor(rank$datatype, levels=c("MET","RNA","restricted","FS"))
...
ggplot(rank, aes(x=datatype, y=values, color=deconv)) +
  ...
  facet_wrap(~feat_selec) +
  scale_color_social_d() +
  ylab("Aggregated score") +
  xlab("") +
  theme_modern(axis.text.angle = 45)
ggsave(paste0("3b_featureselection_step3/221101_plot_",rank_type,"_",rank_norm,".pdf"),
       width=8, height=4)
