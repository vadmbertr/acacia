...
names(A_true) <- input_path_A_true
...
tmp=sapply(seq(n_lot), function (lot) {
  ...
  sapply(seq(length(A_true)/n_lot), function(sim) {
    ...
    sapply(deconv_methods_unsup, function(meth) {
      if (file.exists(paste0("deconv/",block,"/unsup/",strsplit(input_path_T[lot],paste0("_T_",block,"_ref"))[[1]][1],"_Apred_",meth,"_",sim_txt,sim,".rds"))) {
        A_pred=readRDS(paste0("deconv/",block,"/unsup/",strsplit(input_path_T[lot],paste0("_T_",block,"_ref"))[[1]][1],"_Apred_",meth,"_",sim_txt,sim,".rds"))
        if (!all(colnames(A_pred)==col_names)) {
          print(paste0("rerun method ",meth," for sim ",sim," of dataset ",lot," to reorder columns")) 
        }
        if (!all(rownames(A_pred)==row_names[[lot]])) {
