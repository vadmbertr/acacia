...
df_sup <- score_mat_sup %>%
  ...
  summarise(values=mean(trendval, na.rm=T)) %>%
...
df_sup$candidate <- sapply(df_sup$candidate, function(x) strsplit(x, " ")[[1]][1])
df_unsup$facet <- sapply(df_unsup$candidate, function(x) strsplit(x, " ")[[1]][2])
df_unsup$candidate <- sapply(df_unsup$candidate, function(x) strsplit(x, " ")[[1]][1])

plotting(df_sup) +
  geom_line(aes(group=candidate), size=5, alpha=.4) +
  facet_wrap(~facet)
ggsave(paste0("2b_ranking_aggreg/221101_",block,"_",feat_selec,"_sup_ranking_",rank_type,"_",rank_norm,".pdf"),
       width = 8, height = 5)
plotting(df_unsup) +
  geom_line(aes(group=candidate), size=5, alpha=.4) +
  facet_wrap(~facet)
ggsave(paste0("2b_ranking_aggreg/221101_",block,"_",feat_selec,"_unsup_ranking_",rank_type,"_",rank_norm,".pdf"),
       width = 8, height = 5)
