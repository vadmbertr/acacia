...
## Set parameters
## ----
deconv_methods_rna = c("RLR","WISP","DeconRNASeq")
#deconv_methods_rna = c("RLR")
n_meth = length(deconv_methods_rna)
source("../../src/score_functions.R")
source("../../src/ranking_ranking_procedure_functions.R")
ranking_norm=ranking12_notime # to vary, either ranking12_notime or ranking_2_notime
ranking_type=ranking12345_notime # to vary, either ranking12345_notime or ranking12453_notime
rank_norm=ifelse(all.equal(ranking_norm,ranking12_notime)==T,"normDataset","normGlobal")
rank_type=ifelse(all.equal(ranking_type,ranking12453_notime)==T,"order345","order453")
...
score_perf_r = lapply(seq(n_lot), function(lot) {
  lapply(seq(n_sims), function(sim) {
    ...
    #res_avg=matrix(ncol=length(score_methods),nrow=nrow(res[[1]]))
    #for (i in seq(ncol(res_avg)*nrow(res_avg))) {
    #  res_avg[i-(ceiling(i/nrow(res_avg))-1)*nrow(res_avg),ceiling(i/nrow(res_avg))] = mean(sapply(res, function(x) x[i-(ceiling(i/nrow(res_avg))-1)*nrow(res_avg),ceiling(i/nrow(res_avg))]))
    #}
    #return(res_avg)
    })
  })
...
## Compute rankings
## ----
rank <- ranking_type(score_tot,ranking=ranking_norm)
rank$block <- sapply(rank$candidate, function(x)
  strsplit(x, " ")[[1]][2])
rank$deconv <- sapply(rank$candidate, function(x)
  strsplit(x, " ")[[1]][1])
rank$data_type <- sapply(seq_along(rank$block), function(x)
  ifelse(rank$block[x]=="MB","MB",ifelse(rank$deconv[x] %in% c(deconv_methods_rna,"RM"), "RNA", "MET")))

## ----
## Plot
## ----
rank$block <- factor(rank$block, levels=c("SB","MB"))
rank$data_type <- factor(rank$data_type, levels=c("MET","RNA","MB"))
rank$group <- sapply(rank$deconv, function(x) ifelse(x %in% c("MB","MM","RM"),"avg","SB"))
ggplot(rank, aes(x=data_type, y=values, color=deconv)) +
  geom_point() +
  geom_line(data=rank[rank$group=="avg",], aes(group=group), color='black') +
  scale_color_social_d() +
  geom_line(data=rank[rank$group=="avg",], aes(group=group), size=5, alpha=.4, color='black') +
  theme_modern(axis.text.angle = 45)
ggsave(paste0("1_compare/221101_",n_meth,"sup_ranking",rank_type,"_",rank_norm,".pdf"),
       width = 8, height = 5)
