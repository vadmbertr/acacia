...
## Set parameters
## ----
init_type='RsuptoM' # to vary, either MsuptoR, MunsuptoR, RsuptoM or RunsuptoM
...
input_path_method = paste0("2a_init_",init_type,"/")
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    lapply(deconv_methods_in, function(meth_in) {
      lapply(deconv_methods_out, function(meth_out) {
        if (file.exists(paste0("2a_init_",init_type,"/",
                               date,"_",name_data[lot],
                               "_Apred_",meth_in,
                               "_to_",meth_out,
                               "_",sim_txt,sim,".rds"))) {
          readRDS(paste0("2a_init_",init_type,"/",
                                date,"_",name_data[lot],
                                "_Apred_",meth_in,
                                "_to_",meth_out,
    ...
    lapply(deconv_methods_in, function(meth_in) {
      sapply(deconv_methods_out, function(meth_out) {
        if (file.exists(paste0("2a_init_",init_type,"/",
                               ...
                               "_to_",meth_out,
                               "_",sim_txt,sim,".rds"))) {
          tmp=readRDS(paste0("2a_init_",init_type,"/",
                         date,"_",name_data[lot],
                         "_Apred_",meth_in,
                         "_to_",meth_out,
                         "_",sim_txt,sim,".rds"))$time_elapsed
          tmp0=readRDS(paste0("2a_init_",init_type,"/",
                              date,"_",name_data[lot],
                              "_Apred_",meth_in,
                              "_to_",meth_out,
                              "_",sim_txt,sim,".rds"))$res
          if (is.list(tmp)) {
            ...
            saveRDS(list(res=tmp0,time_elapsed=tmp),paste0("2a_init_",init_type,"/",
                               date,"_",name_data[lot],
                               "_Apred_",meth_in,
                               "_to_",meth_out,
                               "_",sim_txt,sim,".rds"))
...
## Compute rankings
## ----
rank <- ranking_type(score_tot1,score_tot2,ranking=ranking_norm)
rank$deconv_in <- sapply(rank$candidate, function(x)
  strsplit(x, " ")[[1]][1])
rank$deconv_out <- sapply(rank$candidate, function(x)
  strsplit(x, " ")[[1]][2])
rank$init <- sapply(rank$deconv_in, function(x) ifelse(x=="no","No","Yes"))
rank_yes <- rank %>%
  filter(init=="Yes")
rank_no <- rank %>%
  filter(init=="No")
for (i in seq(nrow(rank_yes)/length(unique(rank_yes$deconv_out))-1)) {rank_no <- bind_rows(rank_no,rank%>%filter(init=="No"))}
rank <- bind_rows(rank_yes,rank_no)
rm(rank_no)
rank$group <- rep(seq(nrow(rank_yes)),2)

## ----
## Plot
## ----
ggplot(rank, aes(x=init, y=values, color=deconv_in)) +
  geom_point() +
  geom_line(aes(group=group), color="black") +
  scale_color_social_d() +
  facet_wrap(~deconv_out) +
  theme_modern(axis.text.angle = 45)
ggsave(paste0("3_compare/221101_",init_type,"_ranking_",rank_type,"_",rank_norm,".pdf"),
       width = 8, height = 5)
