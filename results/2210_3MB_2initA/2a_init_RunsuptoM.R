...
deconv_methods_RtoM <- c("ICA","RefFreeEWAS","MeDeCom")
...
Dsim = pbapply::pblapply(input_path_matrix, function(x) readRDS(paste0(input_path,x))$D_met_sim)

input_path = paste0("../2210_0simu/simulations/",block1,"/")
input_path_matrix = sort(list.files(input_path, pattern = "sim"))
Atrue = pbapply::pblapply(input_path_matrix, function(x) readRDS(paste0(input_path,x))$A_ref)
...
## ----
## load Apred from RNA unsup
## ----
input_path_methods_sup = paste0("../2210_1SB/deconv/",block1,"/unsup/")
...
tmp=lapply(seq(n_lot), function (lot) {
  ...
  lapply(seq(n_sims), function(sim) {
    ...
    lapply(deconv_methods_in, function(methR) {
      lapply(deconv_methods_RtoM, function(methM) {
        if (!file.exists(paste0("2a_init_RunsuptoM/",
                                strsplit(input_path_T[lot],pattern1)[[1]][1],
                                "Apred_",
                                methR,
                                "_to_",
          ...
          saveRDS(deconv_RtoM,paste0("2a_init_RunsuptoM/",
                                     strsplit(input_path_T[lot],pattern1)[[1]][1],
                                     "Apred_",
                                     methR,
                                     "_to_",
