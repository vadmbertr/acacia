...
cat_meth = c("DECONica","DECONnmf")
names(cat_meth) = deconv_methods_2
...
for (lot in seq(n_lot)) {
  ...
  for (sim in seq(n_sims)) {
    ...
    for (methR in deconv_methods_2) {
      Apred_bl[[name_data[lot]]][[sim]][[methR]] = list()
      Apred_to[[name_data[lot]]][[sim]][[methR]] = list()
      for (methM in deconv_methods_1) {
        if (file.exists(paste0("2a_init_",setting,"/",
          ...
          A_tot=readRDS(paste0("2a_init_",setting,"/",
                               ...
                               "_",sim_txt,
                               sim,".rds"))$res
          Apred_to[[name_data[lot]]][[sim]][[methR]][[methM]] = A_tot
          Apred_bl[[name_data[lot]]][[sim]][[methR]][[methM]] = readRDS(paste0("../2210_1SB/deconv/rna/unsup/",
                                                                               strsplit(input_path_T[lot],pattern2)[[1]][1],
                                                                               "_Apred_",
                                                                               cat_meth[methR],
                                                                               "_",sim_txt,
                                                                               sim,".rds"))
          if (all(is.na(A_tot))) {Apred_to[[name_data[lot]]][[sim]][[methR]][[methM]] = matrix(NA,ncol=n_samples,nrow=n_cell_types[lot])}
        }
...
ggplot(df_rmse_CN, aes(x=CN, y=RMSEdelta, color=Dataset, shape=From)) +
  geom_point() +
...
ggsave(paste0("2b_eval_",setting,"/",
              ...
              "_CN_RMSE1.pdf"),
       width = 8, height = 4)
ggplot(df_rmse_CN, aes(x=CN, y=RMSEdelta, color=Dataset, shape=To)) +
  geom_point() +
...
ggsave(paste0("2b_eval_",setting,"/",
              ...
              "_CN_RMSE2.pdf"),
       width = 12, height = 6)
ggplot(df_rmse_CN, aes(x=CN, y=RMSEdelta, color=Dataset)) +
  geom_point() +
